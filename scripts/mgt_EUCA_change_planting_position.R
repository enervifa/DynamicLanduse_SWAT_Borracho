...
has_planting_operations <- function(year_lines, year_number) {
  ...
  for (line in year_lines) {
    ...
    if (length(columns) >= 3 && columns[4] == "1") {
...
for (i in seq_along(list_of_files)) {
  ...
  if (any(grepl("EUCA", file_content))) {
    ...
    if (length(management_start) > 0) {
      ...
      #browser()
      # Define target years
      target_years<- c(5,10,8,11)
      target_years1<-sample(target_years,length(planting_years)/2)
      target_years2<-case_when(
        target_years1==5 ~ 15,
        target_years1==10 ~ 23,
        target_years1==8 ~ 16,
        target_years1==11 ~ 18)
      # Randomly select a target year
      selected_years <- c(target_years1, target_years2)
      result_years_data <- swap_planting_years(years_data , planting_years, selected_years)
      #browser()
      ...
      if (length(target_line_index) > 0) {
        ...
        replacement_value <- switch(as.character(selected_years[[1]]), #5,10,8,15
                                    "1" ="        200000.00    | BIO_INIT: Initial biomass",
                                    "2" = "       170000.00    | BIO_INIT: Initial biomass",
                                    "3" = "       150000.00    | BIO_INIT: Initial biomass",
                                    "5" = "        10000.00    | BIO_INIT: Initial biomass",
                                    "10" = "      100000.00    | BIO_INIT: Initial biomass",
                                    "8" = "       120000.00    | BIO_INIT: Initial biomass",
                                    "11" = "      100000.00    | BIO_INIT: Initial biomass",
        ...
        ###Modify EUCA crop so it represents to 2018 72 crop 119, 25% crop 184 AND and 3% 183
        
        # Identify the line that contains "1000.00    | BIO_INIT: Initial biomass"
        target_line_index <- grep("  5  1           1  184      1   1800.00000   2.50  1000.00000 0.00   0.00  0.00", modified_content)
        #browser()
      }
        # Replace the value based on selected_years[[1]]
        if (length(target_line_index) > 0) {
          # Sample a number with specified probabilities
          sampled_number <- sample(c(1, 2, 3), 1, prob = c(0.25, 0.72, 0.03))
          
          # Define the replacement value based on the sampled number
          replacement_value <- case_when(
            sampled_number == 1 ~ "  5  1           1  184      1   1800.00000   2.50  10000.00000 0.00   0.00  0.00",
            sampled_number == 2 ~ "  5  1           1  119      1   1800.00000   2.50  10000.00000 0.00   0.00  0.00",
            sampled_number == 3 ~ "  5  1           1  183      1   1800.00000   2.50  10000.00000 0.00   0.00  0.00"
          )
          # Replace the value in the target line
          modified_content[target_line_index] <- replacement_value
          
          ###second seedling given map 2021
          
          # Identify the line that contains "1000.00    | BIO_INIT: Initial biomass"
          target_line_index <- grep("  8 20           1  184      1   1800.00000   1.00    50.00000 0.00   0.00  0.00", modified_content)
          #browser()
        }
          # Replace the value based on selected_years[[1]]
          if (length(target_line_index) > 0) {
            # Sample a number with specified probabilities
            sampled_number <- sample(c(1, 2, 3), 1, prob = c(0.06, 0.90, 0.04))
            
            # Define the replacement value based on the sampled number
            replacement_value <- ifelse(
              sampled_number == 1, 
              "  8 20           1  184      1   1800.00000   2.50   10000.00000 0.00   0.00  0.00",
              ifelse(
                sampled_number == 2,
                "  8 20           1  119      1   1800.00000   2.50   10000.00000 0.00   0.00  0.00",
                "  8 20           1  183      1   1800.00000   2.50   10000.00000 0.00   0.00  0.00"
              )
            ) # Replace the value in the target line
            modified_content[target_line_index] <- replacement_value
          }  
...
setwd('C:/Users/Lenovo/Documents/Borracho_April2024/Borracho/Scenarios/prueba_mgt/EUCA_random')
